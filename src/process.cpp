...
Process::Process(const int pid, const int cycles, const int maxQuantum,
    ...
    : pid(pid), cycles(cycles), maxQuantum(maxQuantum), initBound(initBound),
      priority(priority), timestamp(0) {}
Process::Process(const int pid, const int cycles, const int maxQuantum,
                 enum resources initBound, enum priorities priority,
                 const int timestamp)
    : pid(pid), cycles(cycles), maxQuantum(maxQuantum), initBound(initBound),
...
// TODO: Rescrever makeCycle para consumir ciclo e nao timestamp
...
// TODO: Rescrever a funco isTerminted pra implementar o ciclo separado do
// timestamp
